...
// ⬇️ Action follows the same shape as your dialog’s server action:
// returns { success, data, errors } and is wired via useActionState
async function requestMagicLinkAction(
  ...
  try {
    ...
    // callbackUrl comes from a hidden input for parity with the dialog pattern
    const callbackUrl = (formData.get("callbackUrl") as string | null) ?? Router.HOME;
    ...
    if (res?.error) {
      return {
        data: { email },
        errors: { _form: ["We couldn't send the magic link. Please try again."] },
      };
    }
